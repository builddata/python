...
计算物品之间的相似度
根据物品的相似度和用户的历史行为给用户推荐生成列表
具体做法：

生成用户-物品倒排表和物品-用户倒排表，方便计算

计算相似度矩阵，也就是对训练集中的每个Item，计算它与其它Item之间的相似度

这个矩阵是稀疏矩阵，计算的一个小技巧就是只计算那些有用户交集的Item，有了之前的倒排表，这一步很容易实现
对用户u做推荐，就是根据他评分列表中的物品，找到最相似的k个物品，然后计算这些物品的兴趣值，
最后返回count个兴趣值最高的物品即可
生成推荐列表
